...
/**
 * Render just the breathing layer source colour and per-LED alpha (coverage *
 * `breatheMix`), without compositing onto any backdrop. Used by the generic
 * layer stack in main.ts where order is user-configurable.
 */
export function renderBreathingLayer(
  out: Float32Array,
  alphaOut: Float32Array,
  t: number,
  cfg: Config
) {
  out.fill(0);
